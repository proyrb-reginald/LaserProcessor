...
uint32_t ltdc_lcd_framebuf[1280][800] __attribute__((
    at(LTDC_FRAME_BUF_ADDR))); /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小 */
#else
uint16_t ltdc_lcd_framebuf[1280][800] __attribute__((
    at(LTDC_FRAME_BUF_ADDR))); /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小 */
//    uint16_t ltdc_lcd_framebuf1[1280][800]
//    __attribute__((at(LTDC_FRAME_BUF_ADDR + 1280 * 800 * 2)));/*
//    使用LTDC层2时使用（默认使用LTDC层1） */
#endif
...
#if LTDC_PIXFORMAT == LTDC_PIXFORMAT_ARGB8888 ||                               \
    LTDC_PIXFORMAT == LTDC_PIXFORMAT_RGB888
uint32_t ltdc_lcd_framebuf[1280][800] __attribute__((section(
    ".bss.ARM.__at_0XC0000000"))); /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小
                                    */
#else
uint16_t ltdc_lcd_framebuf[1280][800] __attribute__((section(
    ".bss.ARM.__at_0XC0000000"))); /* 定义最大屏分辨率时,LTDC所需的帧缓存数组大小
                                    */
#endif
...
uint32_t
    *g_ltdc_framebuf[2]; /* LTDC LCD帧缓存数组指针,必须指向对应大小的内存区域 */
...
void ltdc_draw_point(uint16_t x, uint16_t y, uint32_t color)
{
    ...
    if (lcdltdc.dir) /* 横屏 */
    {
        *(uint16_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.activelayer] +
                      lcdltdc.pixsize * (lcdltdc.pwidth * y + x)) = color;
        *(uint8_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.activelayer] +
                     lcdltdc.pixsize * (lcdltdc.pwidth * y + x) + 2) =
            color >> 16;
    }
    else /* 竖屏 */
    {
...
uint32_t ltdc_read_point(uint16_t x, uint16_t y)
{
    ...
    else /* 竖屏 */
    {
        return *(uint32_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.activelayer] +
                             lcdltdc.pixsize *
                                 (lcdltdc.pwidth * (lcdltdc.pheight - x - 1) +
                                  y)) &
               0XFFFFFF;
    }
 ...
 * @note       (sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex - sx + 1) * (ey -
 * sy + 1) 注意:sx,ex,不能大于lcddev.width - 1; sy,ey,不能大于lcddev.height - 1
 * @param       sx,sy       : 起始坐标
 * @param       ex,ey       : 结束坐标
 * @param       color       : 填充的颜色
 * @retval      无
...
void ltdc_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey,
               uint32_t color)
{
    uint32_t psx, psy, pex,
        pey; /* 以LCD面板为基准的坐标系,不随横竖屏变化而变化 */
    uint32_t timeout = 0;
    uint16_t offline;
    uint32_t addr;
    ...
    offline =
        lcdltdc.pwidth -
        (pex - psx +
         1); /* 行偏移:当前行最后一个像素和下一行第一个像素之间的像素数目 */
...
///**
// * @brief       在指定区域内填充单个颜色
// * @param       (sx,sy),(ex,ey)  : 填充矩形对角坐标,区域大小为:(ex - sx + 1) *
// (ey - sy + 1)
// * @param       color            : 要填充的颜色
// * @retval      无
// */
...
//    以LCD面板为基准的坐标系,不随横竖屏变化而变化 */ uint32_t timeout = 0;
//    uint16_t offline;
//    uint32_t addr;
//
//    if (ex >= lcdltdc.width)
...
//    {
//        color = ((color & 0X0000F800) << 8) | ((color & 0X000007E0) << 5 ) |
//        ((color & 0X0000001F) << 3);
//    }
//
//    /* 配置DMA2D的模式 */
...
//    HAL_DMA2D_Start(&g_dma2d_handle, color, (uint32_t)addr, pex - psx + 1, pey
//    - psy + 1);   /* 开启传输 */ HAL_DMA2D_PollForTransfer(&g_dma2d_handle,
//    1000);                                        /* 传输数据 */
//
//    while((__HAL_DMA2D_GET_FLAG( &g_dma2d_handle,DMA2D_FLAG_TC) == 0) &&
//    (timeout < 0X5000)) /* 等待DMA2D传输完成 */
//    {
//        timeout++;
//    }
 ...
 *              (sx,sy),(ex,ey):填充矩形对角坐标,区域大小为:(ex - sx + 1) * (ey
 * - sy + 1) 注意:sx,ex,不能大于lcddev.width - 1; sy,ey,不能大于lcddev.height -
 * 1
 * @param       sx,sy       : 起始坐标
 * @param       ex,ey       : 结束坐标
 ...
 *              Fr: PLL3的r分频输出时钟频率
 *              Fs:
 * PLL3输入时钟频率,可以是HSI,CSI,HSE等(系统时钟初始化时选择HSE作为PLL的输入时钟源).
 *
 *              假设:外部晶振为25M, pllm = 25 的时候, Fs = 25Mhz，
 * pllm分频后频率 为1Mhz. 例如: 要得到33M的LTDC时钟, 则可以设置: pll3n = 300,
 * pllm = 25, pll3r = 9 Fdclk= ((25 / 25) * 300) / 9 = 33 Mhz
 * @retval      0, 成功;
 *              其他, 失败;
 */
uint8_t ltdc_clk_set(uint32_t pll3n, uint32_t pll3m, uint32_t pll3r)
{
    ...
    if (HAL_RCCEx_PeriphCLKConfig(&periphclk_initure) ==
        HAL_OK) /* 配置像素时钟 */
    {
        return 0; /* 成功 */
    }
 ...
 * @note
 * 此函数必须在ltdc_layer_parameter_config之后再设置.另外,当设置的窗口值不等于面板的尺
...
{
    HAL_LTDC_SetWindowPosition(&g_ltdc_handle, sx, sy,
                               layerx); /* 设置窗口的位置 */
    HAL_LTDC_SetWindowSize(&g_ltdc_handle, width, height,
                           layerx); /* 设置窗口大小 */
 ...
 * @param       pixformat   : 层像素格式. 0,ARGB8888; 1,RGB888; 2,RGB565;
 * 3,ARGB1555; 4,ARGB4444; 5,L8; 6;AL44; 7;AL88
 * @param       alpha       : 层恒定Alpha值, 0,全透明;255,不透明
 * @param       alpha0      : 默认颜色Alpha值, 0,全透明;255,不透明
 * @param       bfac1       : 混合系数1, 4(100),恒定Alpha;
 * 6(101),像素Alpha*恒定Alpha
 * @param       bfac2       : 混合系数2, 5(101),恒定Alpha;
 * 7(111),像素Alpha*恒定Alpha
                                 ...
                                 uint32_t bkcolor)
{
    ...
    playercfg.Alpha =
        alpha; /* 设置层恒定Alpha值,0~255,硬件255分频,255为完全不透明 */
    ...
    playercfg.Backcolor.Blue =
        (uint8_t)bkcolor & 0X000000FF; /* 背景颜色蓝色部分 */
    HAL_LTDC_ConfigLayer(&g_ltdc_handle, &playercfg,
                         layerx); /* 设置所选中的层 */
}
...
void ltdc_init(void)
{
    ...
    else if (lcdid == 0X7084)
    {
        ...
        ltdc_clk_set(
            300, 25,
            9); /* 设置像素时钟
                   33Mhz(如果开双显,需要降低DCLK到18.75Mhz,才会比较好) */
    }
    ...
    if (lcdid == 0X8081)
    {
        g_ltdc_handle.Init.HSPolarity =
            LTDC_HSPOLARITY_AH; /* 水平同步极性:高电平有效 */
    }
    else
    {
        g_ltdc_handle.Init.HSPolarity =
            LTDC_HSPOLARITY_AL; /* 水平同步极性:低电平有效 */
    }

    g_ltdc_handle.Init.VSPolarity =
        LTDC_VSPOLARITY_AL; /* 垂直同步极性:低电平有效 */
    g_ltdc_handle.Init.DEPolarity =
        LTDC_DEPOLARITY_AL; /* 数据使能极性:低电平有效 */
    g_ltdc_handle.State = HAL_LTDC_STATE_RESET;

    if (lcdid == 0X1018 || lcdid == 0X8081)
    {
        g_ltdc_handle.Init.PCPolarity =
            LTDC_PCPOLARITY_IIPC; /* 像素时钟极性:高电平有效 */
    }
    else
    {
        g_ltdc_handle.Init.PCPolarity =
            LTDC_PCPOLARITY_IPC; /* 像素时钟极性:低电平有效 */
    }
    ...
    g_ltdc_handle.Init.AccumulatedHBP =
        lcdltdc.hsw + lcdltdc.hbp - 1; /* 累加水平后沿宽度 */
    g_ltdc_handle.Init.AccumulatedVBP =
        lcdltdc.vsw + lcdltdc.vbp - 1; /* 累加垂直后沿高度 */
    ...
    g_ltdc_handle.Init.TotalWidth = lcdltdc.hsw + lcdltdc.hbp + lcdltdc.pwidth +
                                    lcdltdc.hfp - 1; /* 总宽度 */
    g_ltdc_handle.Init.TotalHeigh = lcdltdc.vsw + lcdltdc.vbp +
                                    lcdltdc.pheight + lcdltdc.vfp -
                                    1;      /* 总高度 */
    ...
    ltdc_layer_parameter_config(0, (uint32_t)g_ltdc_framebuf[0], LTDC_PIXFORMAT,
                                255, 0, 6, 7, 0X000000); /* 层参数配置 */
    ltdc_layer_window_config(
        0, 0, 0, lcdltdc.pwidth,
        lcdltdc.pheight); /* 层窗口配置,以LCD面板坐标系为基准,不要随便修改! */

    // ltdc_layer_parameter_config(1, (uint32_t)g_ltdc_framebuf[1],
    // LTDC_PIXFORMAT, 127, 0, 6, 7, 0X000000); /* 层参数配置 */
...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef *hltdc)
{
    ...
    /* LTDC信号控制引脚 LTDC_DE(PF10), LTDC_VSYNC(PI9), LTDC_HSYNC(PI10),
     * LTDC_CLK(PG7) */
    /* LTDC 数据引脚    LTDC_R7(PG6), LTDC_R6(PH12), LTDC_R5(PH11),
       LTDC_R4(PH10), LTDC_R3(PH9), LTDC_R2(PH8), LTDC_R1(PA2), LTDC_R0(PG13),
                       LTDC_G7(PI2), LTDC_G6(PI1), LTDC_G5(PI0), LTDC_G4(PH15),
       LTDC_G3(PH14), LTDC_G2(PH13), LTDC_G1(PE6), LTDC_G0(PE5), LTDC_B7(PI7),
       LTDC_B6(PI6), LTDC_B5(PI5), LTDC_B4(PI4), LTDC_B3(PA8), LTDC_B2(PD6),
       LTDC_B1(PG12), LTDC_B0(PG14) */
    ...
    gpio_init_struct.Pull = GPIO_PULLUP;         /* 上拉 */
    gpio_init_struct.Speed = GPIO_SPEED_HIGH;    /* 高速 */
    HAL_GPIO_Init(LTDC_BL_GPIO_PORT,
                  &gpio_init_struct); /* LTDC_BL引脚模式设置 */

    gpio_init_struct.Pin = LTDC_RST_GPIO_PIN;
    HAL_GPIO_Init(LTDC_RST_GPIO_PORT,
                  &gpio_init_struct); /* LTDC_RST引脚模式设置 */
    ...
    HAL_GPIO_Init(LTDC_DE_GPIO_PORT,
                  &gpio_init_struct); /* LTDC_DE引脚模式设置 */

    gpio_init_struct.Pin = LTDC_VSYNC_GPIO_PIN;
    HAL_GPIO_Init(LTDC_VSYNC_GPIO_PORT,
                  &gpio_init_struct); /* LTDC_VSYNC引脚模式设置 */

    gpio_init_struct.Pin = LTDC_HSYNC_GPIO_PIN;
    HAL_GPIO_Init(LTDC_HSYNC_GPIO_PORT,
                  &gpio_init_struct); /* LTDC_HSYNC引脚模式设置 */

    gpio_init_struct.Pin = LTDC_CLK_GPIO_PIN;
    HAL_GPIO_Init(LTDC_CLK_GPIO_PORT,
                  &gpio_init_struct); /* LTDC_CLK引脚模式设置 */
