...
typedef struct {
    uint32_t
        pwidth; /* LTDC面板的宽度,固定参数,不随显示方向改变,如果为0,说明没有任何RGB屏接入
                 */
...
#define LTDC_VSYNC_GPIO_CLK_ENABLE()                                           \
    do {                                                                       \
...
#define LTDC_HSYNC_GPIO_CLK_ENABLE()                                           \
    do {                                                                       \
        __HAL_RCC_GPIOI_CLK_ENABLE();                                          \
    } while (0) /* 所在IO口时钟使能 */
...
#define LTDC_CLK_GPIO_CLK_ENABLE()                                             \
    do {                                                                       \
...
#define LTDC_BL_GPIO_CLK_ENABLE()                                              \
    do {                                                                       \
...
#define LTDC_RST_GPIO_CLK_ENABLE()                                             \
    do {                                                                       \
        __HAL_RCC_GPIOD_CLK_ENABLE();                                          \
    } while (0) /* 所在IO口时钟使能 */
...
#define LTDC_BL(x)                                                             \
    do {                                                                       \
        x ? HAL_GPIO_WritePin(LTDC_BL_GPIO_PORT, LTDC_BL_GPIO_PIN,             \
                              GPIO_PIN_SET)                                    \
...
#define LTDC_RST(x)                                                            \
    do {                                                                       \
        x ? HAL_GPIO_WritePin(LTDC_RST_GPIO_PORT, LTDC_RST_GPIO_PIN,           \
                              GPIO_PIN_SET)                                    \
          : HAL_GPIO_WritePin(LTDC_RST_GPIO_PORT, LTDC_RST_GPIO_PIN,           \
                              GPIO_PIN_RESET);                                 \
    } while (0)
